...
import { getUsers, getBills, calculateSettlements, markShareAsPaid } from '../services/dataService';
import { User, Bill, Settlement, CurrencyType, BillStatus } from '../types';
import { Card, Statistic, Row, Col, List, Avatar, Tag, Empty, Spin, Button, App } from 'antd';
import { 
  UserOutlined, 
  FileTextOutlined, 
  TransactionOutlined, 
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const loadData = () => {
      if (typeof window !== 'undefined') {
        ...
        // 处理图表数据
        const userBalances: Record<string, { userId: string, name: string, cny: number, jpy: number }> = {};
        ...
        settlementsData.forEach(settlement => {
          // 确保用户ID存在
          if (userBalances[settlement.fromUser] && userBalances[settlement.toUser]) {
        ...
        billsData.forEach(bill => {
          if (bill.status === BillStatus.PENDING) {
            bill.shares.forEach(share => {
              // 确保用户ID存在
              if (!share.paid && userBalances[share.userId] && userBalances[bill.createdBy]) {
  ...
  const handleSettlementComplete = (settlement: Settlement) => {
    ...
    const relevantBills = bills.filter(bill => 
      ...
      bill.shares.some(share => 
        share.userId === settlement.fromUser && !share.paid
      )
    );
      ...
      <Row gutter={16} style={{ marginBottom: '20px' }}>
        ...
        <Col xs={24} sm={12} md={8} lg={6}>
          <Card>
            <Statistic
              title="流通金额 (人民币)"
              value={bills.filter(b => b.currency === CurrencyType.CNY).reduce((sum, bill) => sum + bill.totalAmount, 0) / 100}
              precision={2}
        ...
        <Col xs={24} sm={12} md={8} lg={6}>
          <Card>
            <Statistic
              title="流通金额 (日元)"
              value={bills.filter(b => b.currency === CurrencyType.JPY).reduce((sum, bill) => sum + bill.totalAmount, 0)}
              precision={0}
      ...
      <Card 
        title={
          <div style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <span style={{ fontWeight: 'bold', color: '#1677ff' }}>结算金额统计</span>
          </div>
        }
