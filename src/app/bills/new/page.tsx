...
export default function NewBill() {
  ...
  const updateShares = (userIds: string[], creator: string) => {
    ...
    const newShares = userIds.map((userId) => {
      const existingShare = prevShares.find(
        (share: BillShare) => share.userId === userId
      );
      return (
        existingShare || {
          userId,
  ...
  const handleSubmit = async (values: Record<string, unknown>) => {
    ...
    const sharesTotal = (values.shares as BillShare[]).reduce(
    ...
    <div style={{ maxWidth: "800px", margin: "0 auto", padding: "24px 16px" }}>
      ...
      >
        ...
        >
          ...
          <Form.Item name="description" label="账单描述">
                  ...
                  return (
                    ...
                    >
                      <div style={{ width: 150, marginRight: 16 }}>
                        <Tag
                          icon={<UserOutlined />}
                          color={userId === createdBy ? "blue" : "default"}
                        >
                          {getUserName(userId)}{" "}
                          {userId === createdBy && "(创建者)"}
                        </Tag>
                      </div>
                      <Form.Item
                        {...restField}
          ...
          <Form.Item>
            <Space>
              ...
              <Button onClick={() => router.push("/bills")}>取消</Button>
            </Space>
          </Form.Item>
        </Form>
      </Card>
